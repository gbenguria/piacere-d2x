...
public class Domlx2domlService implements Domlx2domlApiDelegate {
    ...
    public ResponseEntity<String> domlx2domlPost(String body) {
        String sourceExt = "domlx";
        String targetExt = "doml";
        ...
        try{
            ...
            e.printStackTrace();
            return new ResponseEntity<>("Error while loading domlx into ecore resource", HttpStatus.INTERNAL_SERVER_ERROR);
        }

        logger.info("domlx loaded into ecore resource");

        // print inputResource errors
        for (Resource.Diagnostic domlError : inputResource.getErrors()) {
            logger.error("Domlx error: " + domlError.getMessage());
        }
        ...
        try {
            ...
            e.printStackTrace();
            return new ResponseEntity<>("Error while creating doml from ecore resource", HttpStatus.INTERNAL_SERVER_ERROR);
        }
        logger.info("doml created from ecore resource");
        ...
        try {
            ...
            return new ResponseEntity<>("Error while loading doml into string: \n" + e.getMessage() + "\nStackTrace:\n" + sw, HttpStatus.INTERNAL_SERVER_ERROR);
        }

        logger.info("doml loaded into string");
        String doml = outputStream.toString();

        // return doml
        return new ResponseEntity<>(doml, HttpStatus.CREATED);
    }
